...
function AddNewRentSheduleSelection() {
  ...
  const handleRentingFromTimeAdd = (index) => {
    const newRentingDateList = [...rentingDateList];
    newRentingDateList[index].rentingTimes.push({ RentingFromTime: "", RentingToTime: "" });
    setrentingDateList(newRentingDateList);
  };
              ...
              <div key={index+ "-" + innerIndex} className="services">
                <div className="first-division">
                  ...
                  <input
                    ...
                    onChange={(e) => handleRentingTimeChange(e, index, innerIndex)}
                    required
                  />
                  <span> To </span>
                  <input
                    ...
                    required
                  />
                  {rentingDateList[index].rentingTimes.length - 1 === innerIndex && (
                    <button
                      type="button"
                ...
                <div className="second-division">
                  {rentingDateList[index].rentingTimes.length !== 1 && (
                    <button
                      type="button"
                      onClick={() => handleRentingFromTimeRemove(index, innerIndex)}
                      className="remove-btn"
                    >
                      <span>Remove</span>
                    </button>
